...
int main_ini(void)
{
	...
	//Set CRC Type
	// UserSetCrcType(ECM_CRC_TYPE_32);
	// ECM_SetCRCTypet(g_u32CrcType);

	// if (ECM_InitLibrary() == 0)
	// {
	// 	PRINTF("ECM_InitLibrary fail\r\n");
	// 	return -1;
	// }
	// if (ECM_SetSpiDataSize(TEST_SPI_DATA_SIZE) == 0)
	// {
	// 	PRINTF("ECM_SetSpiDataSize wait async done time out\r\n");
	// 	return -1;
	// }
	// ECM_GetSpiDataSize(&u16PDOSize);
	// PRINTF("SPI data size %d\r\n", u16PDOSize);
	// nret = ECM_GetFirmwareVersion(&u8Data);
	// if (nret <= 0)
	// 	return -1;
	// PRINTF("ECM-XF Firmware version : 0x%X\r\n", u8Data);
	// nret = ECM_GetTxFIFOCnt(&u8Data);
	// if (nret <= 0)
	// {
	// 	return -1;
	// }
	// PRINTF("Get TxFIFOCnt %d\r\n", u8Data);
	// nret = ECM_GetRxFIFOCnt(&u8Data);
	// if (nret <= 0)
	// {
	// 	return -1;
	// }
	// PRINTF("Get RxFIFOCnt %d\r\n", u8Data);
	// ECM_EnableFIFO(0);
	// nret = ECM_SetTxFIFOCnt(TEST_FIFO_CNT_MAX);
	// if (nret <= 0)
	// {
	// 	PRINTF("ECM_SetTxFIFOCnt %d\r\n", nret);
	// 	return -1;
	// }
	// nret = ECM_SetRxFIFOCnt(TEST_FIFO_CNT_MAX);
	// if (nret <= 0)
	// {
	// 	PRINTF("ECM_SetRxFIFOCnt %d\r\n", nret);
	// 	return -1;
	// }
	// nret = ECM_GetTxFIFOCnt(&u8Data);
	// if (nret <= 0)
	...
	nret = ECM_EcatInit(0x0300, TEST_CYCTIME, TEST_CYCTIME / 2); //Ó¦¸ÃÊÇ0x0003°É£¿
	...
	nret = ECM_StateCheck(0xFF, EC_STATE_PRE_OP, 2000); // Set mode must be at PRE-OP state
	...
	for (i = 0; i < TEST_SERVO_CNT; i++)
	{
		...
		SetPdoConfTbl(&RxPDOConfig[i], 0, 1, 0x607A, 0, 32); //0x607A:0 - target position // 32 bits = 4 bytes for TEST_RXPDO_SIZE
															 //        SetPdoConfTbl(&RxPDOConfig[i], 0, 2, 0x60B8, 0, 16); //0x60B8:0 - TouchProbeFunction // 16 bits = 2 bytes for TEST_RXPDO_SIZE
		...
		SetPdoConfTbl(&TxPDOConfig[i], 0, 1, 0x6064, 0, 32); //0x6064:0 - actual position // 32 bits = 4 bytes for TEST_TXPDO_SIZE
															 //        SetPdoConfTbl(&TxPDOConfig[i], 0, 2, 0x60B9, 0, 16); //0x6064:0 - actual position // 32 bits = 4 bytes for TEST_TXPDO_SIZE
															 //        SetPdoConfTbl(&TxPDOConfig[i], 0, 3, 0x60BA, 0, 32); //0x6064:0 - actual position // 32 bits = 4 bytes for TEST_TXPDO_SIZE
															 //        SetPdoConfTbl(&TxPDOConfig[i], 0, 4, 0x60BC, 0, 32); //0x6064:0 - actual position // 32 bits = 4 bytes for TEST_TXPDO_SIZE
															 //        SetPdoConfTbl(&TxPDOConfig[i], 0, 5, 0x603F, 0, 16); //0x6064:0 - actual position // 32 bits = 4 bytes for TEST_TXPDO_SIZE
															 //        SetPdoConfTbl(&TxPDOConfig[i], 0, 6, 0x60FD, 0, 32); //0x6064:0 - actual position // 32 bits = 4 bytes for TEST_TXPDO_SIZE
	}
	...
	if (nret == 0)
	{
		PRINTF("Can't change to OP state\r\n");
		return -1;
	}
