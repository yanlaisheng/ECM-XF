...
int ECM_EcatInit(uint16_t DCAssignActivate, uint32_t CycTime, int32_t CycShift)
{
	...
	大小端
	这个自然也是很有影响的，如果写的时候默认小端模式，而芯片中却是大端模式，那样数据就会直接读成倒的了，如数组{0x1234，0x5678} ，强制转换成结构体指针后，假设默认是小端模式，作者认为结构体中两个数据应该为{data1 = 0x1234, data2 = 0x5678} ，而芯片实际是大端模式，那CPU读取的数据就是{data1 = 0x7856, data2 = 0x3412} 
	*/
	...
	return EcmCmdTransceiver_WaitAyncDone(10000, 0, 0);
...
/*
 * Return
 * 	0	:	ECM information update fail
 * 	1	:	Success
 *
 * */
int ECM_InitLibrary(void)
{
	int i = 0;
...
int ECM_SetSpiDataSize(uint16_t u16SpiDataSize)
{
	ECM_PACK_BEGIN
	typedef struct ECM_PACK
	{
		SPI_CMD_HEADER Head;
		uint8_t Data[PKG_DATA_DEFAULT_SIZE];
		uint32_t Crc;
		uint32_t StopWord;
	} SPI_CMD_DEFAULT_PACKAGE_T;
	ECM_PACK_END
	if (ECM_WaitAsyncDone(10000) == 0)
	{
		return 0;
	}
	SPI_CMD_DEFAULT_PACKAGE_T *pDefaultCmd = (SPI_CMD_DEFAULT_PACKAGE_T *)u8TxBuf;
	pDefaultCmd->Head.u8Cmd = ECM_CMD_SPI_RECONFIG_OP;
	pDefaultCmd->Head.u16Size = u16SpiDataSize;
	pDefaultCmd->Head.u8Idx++;
	pDefaultCmd->Head.u32StartWord = ECM_START_WORD;
	pDefaultCmd->Crc = ECM_CRC_MAGIC_NUM;
	pDefaultCmd->StopWord = ECM_STOP_WORD;
	UserSpiDataExchange(u8TxBuf, u8RxBuf, PKG_DEFAULT_SIZE);
	// UserDelay(100000); //100ms
	HAL_Delay(100);
	return 1;
}

int ECM_GetSpiDataSize(uint16_t *pu16SpiDataSize)
{
...
int ECM_SetTxFIFOCnt(uint8_t u8TxCnt)
{
	pCmd->Head.u8Cmd = ECM_CMD_FIFO_SET_TX_CNT;
	pCmd->Head.u16Size = 0;
	pCmd->Head.u8Idx++;
	pCmd->Head.u8Param = u8TxCnt;
	return EcmCmdTransceiver(1000);
}
int ECM_SetRxFIFOCnt(uint8_t u8RxCnt)
{
...
int ECM_GetTxFIFOCnt(uint8_t *pu8Cnt)
{
	int nret;
...
int ECM_GetRxFIFOCnt(uint8_t *pu8Cnt)
{
	int nret;
...
int ECM_EnableFIFO(uint8_t u8Enable)
{
	pCmd->Head.u8Cmd = ECM_CMD_FIFO_ENABLE_OP;
	pCmd->Head.u16Size = 0;
	pCmd->Head.u8Idx++;
...
int SpiDataExchange(uint8_t *RetIdx, uint8_t *RetCmd)
{
	...
	{
		PRINTF("g_u8CrcErrConti reached MAX(%d)\r\n", SPI_CRC_ERROR_CONTI_MAX);
		return 0;
	}
...
//	HAL_Delay(1);
	...
	{
		PRINTF("CRC Error(Idx : %d) - (Cmd : %d)\r\n", pCmd->Head.u8Idx, pCmd->Head.u8Cmd);
	}
	else
	{
		PRINTF("shift (%d)\r\n", sizeof(SPI_CMD_PACKAGE_T) - i - 4);
 ...
 * */
int EcmCmdTransceiver(int nTryCnt)
{
	int i;
	uint8_t IdxCheck, CmdCheck;
	for (i = 0; i < nTryCnt; i++)
	{
		if (SpiDataExchange(&IdxCheck, &CmdCheck))
		{
			if (pCmd->Head.u8Idx == IdxCheck)
			{
 ...
 *			-3	:	Param error
 * */
int EcmCmdTransceiver_WaitAyncDone(int nTryCnt, uint16_t *pu16ReturnDataSize, uint8_t *pu8ReturnData)
{
	int nret = 0;
